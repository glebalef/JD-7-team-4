    ...
    public int process(List<Update> updates) {
        try{  updates.forEach(update -> {
                    ...
                    // Process your updates here
                        ...
                        //поиск объекта(PersonDog или PersonCat) из таблицы Context по ChatId
context = contextRepository.findByChatId(update.message().chat().id());
                       ...
                       switch (Objects.requireNonNull(parse(update.message().text()))) {
                            ...
                            case BACK_TO_CHOOSE_SHELTER:
                                //запись объекта в таблицу Context
                            ...
                            case CAT_SHELTER:
                                ...
                                telegramBot.execute(replyMessages.generalInfoMessageCat(update)
                                        .replyMarkup(keyboards.getInitialKeyboard()));
                                break;
                            //возврат к первому меню выбранного приюта
                            ...
                            //возврат к меню информации об усыновлении
                            case HOW_TO_ADOPT:
                            case BACK_TO_ADOPT:
            ...
            //для ответа волонтера на сообщение об окончании испытательного срока у усыновителя

                //информация для InlineKeyboardButton сообщения из метода sendNotificationDog
                CallbackQuery callbackQuery = update.callbackQuery();
                String data = callbackQuery.data();
                //перебор выбранных отчетов из метода sendNotificationDog
        ...
        try {
            ...
            for (DogReport dogReport : schedulerService.findOldDogReports()) {
                ...
                if (dogReport.getReportDate().equals(LocalDate.now().minusDays(44))
                    ...
                    //запись отчета в коллекцию выбранных отчетов
                    reports.add(dogReport);
